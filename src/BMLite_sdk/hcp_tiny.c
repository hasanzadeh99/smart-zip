...
fpc_bep_result_t bmlite_init_cmd(HCP_comm_t *hcp_comm, uint16_t cmd, uint16_t arg_key)
{
    ...
    if(arg_key != ARG_NONE) {
        ...
        if(bep_result) {
            ...
            printk("hcp_tiny75BMLITE_ERROR_SEND_CMD");
            return bep_result;
        }
    }    

    // printk("hcp_tiny75FPC_BEP_RESULT_OK 1300 \n");
...
fpc_bep_result_t bmlite_add_arg(HCP_comm_t *hcp_comm, uint16_t arg_type, void *arg_data, uint16_t arg_size)
{
    if(hcp_comm->pkt_size + 4 + arg_size > hcp_comm->pkt_size_max) {
        ...
        printk("hcp_tiny83FPC_BEP_RESULT_NO_MEMORY\n");
    ...
    // printk("hcp_tiny83FPC_BEP_RESULT_OK2\n");   
    return FPC_BEP_RESULT_OK;
}
...
fpc_bep_result_t bmlite_copy_arg(HCP_comm_t *hcp_comm, uint16_t arg_key, void *arg_data, uint16_t arg_data_size)
{
    ...
    if(bep_result == FPC_BEP_RESULT_OK) {
        if(arg_data == NULL) {
            ...
            printk("132 BMLITE_ERROR_GET_ARG  %d",FPC_BEP_RESULT_NO_MEMORY );
            return FPC_BEP_RESULT_NO_MEMORY;
        }
        memcpy(arg_data, hcp_comm->arg.data, HCP_MIN(arg_data_size, hcp_comm->arg.size));
    } else {
        // bmlite_on_error(BMLITE_ERROR_GET_ARG, FPC_BEP_RESULT_INVALID_ARGUMENT);
        printk(" 138 BMLITE_ERROR_GET_ARG  %d",FPC_BEP_RESULT_INVALID_ARGUMENT );
...
fpc_bep_result_t bmlite_receive(HCP_comm_t *hcp_comm)
{
    ...
    while(seq_nr < seq_len) {
        ...
        if (!bep_result) {
            ...
            if (seq_len > 1)
                printk("hcp_tiny197Received data chunk %d of %d\n", seq_nr, seq_len);

        } else {
            // bmlite_on_error(BMLITE_ERROR_SEND_CMD, bep_result);
            printk("hcp_tiny201bmlite_receive bmlite_on_error returned %d \n", bep_result);
            return bep_result;
        }
    }
    ...
    if(com_result != FPC_BEP_RESULT_OK) {
        ...
        printk("hcp_tiny206bmlite_receive BMLITE_ERROR_SEND_CMD %d \n", com_result);
    }
    return com_result;
}
...
static fpc_bep_result_t _rx_link(HCP_comm_t *hcp_comm)
{
    ...
    if (result) {
        printk("hcp_tiny219Timed out waiting for response.\n");
        return result;
    }
    ...
    if (MTU < size + 8) {
        ...
        printk("hcp_tiny229BMLITE_ERROR_SEND_CMD Invalid size %d", FPC_BEP_RESULT_IO_ERROR);
    ...
    if (crc_calc != crc) {
        printk("hcp_tiny239CRC mismatch. Calculated %04X, received %04X\n", 
...
fpc_bep_result_t bmlite_send(HCP_comm_t *hcp_comm)
{
    ...
    if(bep_result) {
        ...
        printk("hcp_tiny286BMLITE_ERROR_SEND_CMD on bmlite_send \n");
        // bmlite_on_error(BMLITE_ERROR_SEND_CMD, bep_result);
    }
    return bep_result;
}
...
static fpc_bep_result_t _tx_link(HCP_comm_t *hcp_comm)
{
   ...
   bep_result = hcp_comm->read(4, (uint8_t *)&ack, 500);
    ...
    if(ack != fpc_com_ack) {
        printk("hcp_tiny318FPC_BEP_RESULT_IO_ERROR\n");
        return FPC_BEP_RESULT_IO_ERROR;
    }
